...
def arm_invk(o_body, frame_body, o_head, frame_head):
    ...
    # scissor joint = delta_origin_without_z - default_offset
    if abs(delta_o[0, 0]) < 1e-9 and abs(delta_o[1, 0]) < 1e-9:
        # avoid the norm function from dividing by zero
        scissor_ext = 0
    else:
        # scissor joint = delta_origin_without_z - default_offset
        scissor_ext = np.linalg.norm(delta_o - [[0.], [0.], delta_o[2]]) - constant.SCISSOR_DISPLACEMENT
    ...
    return np.array([[arm_ext], [scissor_ext], [pan], [tilt]])
